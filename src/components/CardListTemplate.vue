    ...
    <section class="border border-gray-300 flex flex-col rounded-lg items-center w-60">
        <article class="" @click="navigateToProduct">
            ...
            <figcaption class="flex flex-col ">
                <h4 class="text-black text-base my-2">{{ props.item.name}}</h4>                
            </figcaption>
        </article>
        <div class="block w-full py-2">
            <div class="flex w-full justify-end pr-8"><h4 class="text-yellow-500 text-lg">${{ props.item.price}}</h4></div>
            <div class="flex px-2 gap-4 justify-between w-full">
                <btn-add-less :item="props.item"/>
                <button class="bg-red-500 px-5 py-2 rounded-lg text-white" @click="navigateToCart()">Añadir</button>
            </div>
        </div>
    </section>
</template>
